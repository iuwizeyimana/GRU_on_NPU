...
kernels := mm add mul mulmin1 tanhadd sigadd
...
KERNEL_DEFINES_MM := -D${dtype_in}_${dtype_out}_ONLY -DDIM_M=${m} -DDIM_K=${k} -DDIM_N=${n}
KERNEL_DEFINES_E := -DDIM_M=${m} -DDIM_N=${n}
...
# -------- aie args ----------------
aieargs += --dev ${devicename} -M ${M} -K ${K} -N ${N} 
...
build/mm.o: mm.cc
	mkdir -p ${@D}
	cd ${@D} && ${KERNEL_CC} ${KERNEL_CFLAGS} ${KERNEL_DEFINES_MM} -c $< -o ${@F}

build/add.o: add.cc
	mkdir -p ${@D}
	cd ${@D} && ${KERNEL_CC} ${KERNEL_CFLAGS} ${KERNEL_DEFINES_E} -c $< -o ${@F}

build/tanhadd.o: tanhadd.cc
	mkdir -p ${@D}
	cd ${@D} && ${KERNEL_CC} ${KERNEL_CFLAGS} ${KERNEL_DEFINES_E} -c $< -o ${@F}

build/sigadd.o: sigadd.cc
	mkdir -p ${@D}
	cd ${@D} && ${KERNEL_CC} ${KERNEL_CFLAGS} ${KERNEL_DEFINES_E} -c $< -o ${@F}
...
run: ${host_out} ${xclbin_target}
	...
	${powershell} ./${host_out} -x ${xclbin_target} -i ${insts_target} -k MLIR_AIE -M ${M} -K ${K} -N ${N} ${runargs}

#-------------------- Trace -----------------
trace: ${host_out} ${trace_xclbin_target}  ${insts_target}
	export XRT_HACK_UNSECURE_LOADING_XCLBIN=1 && \
	${powershell} ./${host_out} -x ${trace_xclbin_target} -i ${insts_target} -k MLIR_AIE -M ${M} -K ${K} -N ${N} ${runargs} -t ${trace_size}
