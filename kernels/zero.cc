...
void store_vectorized(T *__restrict c_in, T *__restrict c_out) {
  constexpr int r = 512 / (sizeof(T)*8); // 512 bit store units for AIE2P
  const T *__restrict c_end = c_in + M * N;
  event0();
  for(; c_in < c_end; c_in += r){
    aie::vector<T, r> input_ = aie::load_v<r>(c_in);
    aie::store_v(c_out, input_);
    c_out += r;
  }  
  event1();
}
