...
matmul_vectorized_8x8x8_bf16_bf16(const bfloat16 *__restrict pA,
  ...
  return matmul_vectorized_2x2_mmul<bfloat16, bfloat16, (m / r), (k / s),
                                    (n / t), r, s, t>(pA, pB, pC);
  //return matmul_vectorized_2x2_bfp16<(m / r), (k / s), (n / t), r, s, t>(pA, pB, pC);
}
...
#define store_vectorized_c_func(ctype_in, mlir_type_in, ctype_out,              \
                               mlir_type_out, r, s, t)                         \
  void store_##mlir_type_out(ctype_out *c_in, ctype_out *c_out) {                                \
    store_vectorized<ctype_out, DIM_M, DIM_N>(c_in, c_out);                           \
  }

combos(matmul_vectorized_c_func) combos(matmul_scalar_c_func)
    combos(zero_vectorized_c_func) combos(zero_scalar_c_func)
       combos(store_vectorized_c_func)
